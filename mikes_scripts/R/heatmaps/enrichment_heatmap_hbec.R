...
basepath <- "C:/kutluaylab/data/HBEC_162_163/riboseq/"
...
### Vero ribo only ### (may not be needed for github)
...
# For example, change the first letter of each word to upper case, or change all abbreviations to upper case.
...
h <- Heatmap(em.q.values,
             ...
             cell_fun = function(j, i, x, y, width, height, fill) {
               ...
               grid.circle(x = x, y = y,
                           ...
                           # r = unit(sqrt(ratio.values[i, j]) * 0.3, units = "npc"),
                           default.units = "mm",
...
ratio_legend <- Legend(labels = ratio_breaks,
                       ...
                       # size of ratio legend breaks circles
                       # IMPORTANT: change the multiplier number so that the legend circle sizes match the sizes of circles in the heatmap grids.
